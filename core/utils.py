...
def init_web3(config=None):
    try:
        ...
        logging.info(f"[Web3] Connected to {infura_url}")
        return w3
...
def load_oracle(w3, config):
    try:
        oracle_address = config.get("ORACLE", "address", fallback=None)
        ttl = config.getint("ORACLE", "ttl", fallback=60)
        return Oracle(w3=w3, address=oracle_address, ttl=ttl)
    except Exception as e:
...
def estimate_gas_price(w3):
    try:
        ...
        return decimal.Decimal(gas_price_wei) / decimal.Decimal(1e18)
    except Exception as e:
...
def get_min_out(w3, token_in, token_out, amount_in, fee=3000, slippage=0.01):
    try:
        ...
        logging.info(f"[Quote] Expected={quoted}, MinOut={min_out} (slippage {slippage:.2%})")
        return min_out
    except Exception as e:
...
def execute_swap(w3, wallet, router, token, action, gas_price, fee=3000, slippage=0.01):
    try:
        ...
        logging.info(f"[Swap] {action.upper()} executed: TX = {tx_hash.hex()}")
        return tx_hash.hex()
